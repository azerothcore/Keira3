...
describe('SpellLootTemplate integration tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [ToastrModule.forRoot(), ModalModule.forRoot(), SpellLootTemplateComponent, RouterTestingModule, TranslateTestingModule],
      providers: [
  ...
  describe('Creating new', () => {
    ...
    it('schema sweep: every editable field flows into the diff query', async () => {
      ...
      const written = await page.changeAllFieldsAsync(new SpellLootTemplate(), ['Entry']);
    ...
    it('adding a row changing its values and duplicate it should correctly update the queries', () => {
      ...
      page.expectDiffQueryToContain(
        'DELETE FROM `spell_loot_template` WHERE (`Entry` = 1234) AND (`Item` IN (123, 0));\n' +
          'INSERT INTO `spell_loot_template` (`Entry`, `Item`, `Reference`, `Chance`, `QuestRequired`, `LootMode`, ' +
          '`GroupId`, `MinCount`, `MaxCount`, `Comment`) VALUES\n' +
          "(1234, 123, 0, 1, 2, 1, 0, 1, 1, ''),\n" +
          "(1234, 0, 0, 1, 2, 1, 0, 1, 1, '');",
