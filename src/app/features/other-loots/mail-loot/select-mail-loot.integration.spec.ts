...
describe('SelectMailLoot integration tests', () => {
  ...
  it('should correctly initialise', waitForAsync(async () => {
    ...
    expect(querySpy).toHaveBeenCalledWith(
      'SELECT MAX(Entry) AS max FROM mail_loot_template;'
    );
    expect(page.queryWrapper.innerText).toContain(
      'SELECT `Entry` FROM `mail_loot_template` GROUP BY Entry LIMIT 50'
    );
  }));
  ...
  it('should correctly behave when inserting and selecting free entry', waitForAsync(async () => {
    ...
    expect(querySpy).toHaveBeenCalledWith(
      `SELECT * FROM \`mail_loot_template\` WHERE (Entry = ${value})`
    );
    page.expectNewEntityFree();
  ...
  for (const { id, entry, limit, expectedQuery } of [
    {
      id: 1, entry: 1200, limit: '100', expectedQuery:
        'SELECT `Entry` FROM `mail_loot_template` ' +
  ...
  it('searching and selecting an existing entity from the datatable should correctly work', () => {
    ...
    const results: Partial<MailLootTemplate>[] = [
      { Entry: 1 },
      { Entry: 2 },
      { Entry: 3 },
    ];
